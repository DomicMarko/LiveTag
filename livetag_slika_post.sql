...
DROP TABLE IF EXISTS `slika_post`;
...
CREATE TABLE `slika_post` (
  ...
  CONSTRAINT `TopikSlike` FOREIGN KEY (`TopikID`) REFERENCES `topik` (`TopikID`) ON DELETE NO ACTION ON UPDATE CASCADE,
  CONSTRAINT `Vlasnik` FOREIGN KEY (`KorisnikID`) REFERENCES `korisnik` (`KorisnikID`) ON DELETE CASCADE ON UPDATE CASCADE
...
LOCK TABLES `slika_post` WRITE;
/*!40000 ALTER TABLE `slika_post` DISABLE KEYS */;
/*!40000 ALTER TABLE `slika_post` ENABLE KEYS */;
UNLOCK TABLES;
